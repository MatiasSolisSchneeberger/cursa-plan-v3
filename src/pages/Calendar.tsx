...
export default function Calendar() {
		...
		async function fetchData() {
			...
			try {
				...
				const periods = Array.from(
					new Set(
						transformedEvents.map((e) => e.period).filter(Boolean),
					),
				) as string[];
				if (periods.length > 0) {
					const defaultPeriod =
						periods.find((p) =>
							p.toLowerCase().includes("cuatrimestre"),
						) || periods[0];
	...
	return (
		<section className="flex flex-col gap-3">
			...
			<Tabs
				defaultValue="feriados"
				value={tipoCalendario}
				onValueChange={(val) => setTipoCalendario(val)}
			>
							...
							>
								...
								{availablePeriods.map((period) => (
									<TabsContent key={period} value={period}>
										<section
											className={cn(
												"grid grid-cols-1 items-center justify-center justify-items-center gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4",
												loading
										...
										>
											...
											{Array.from(
												{ length: 12 },
												(_, i) => {
													const mesFecha = new Date(
														2026,
														i,
														1,
													); // Año hardcodeado o dinámico según necesites
													...
													return (
														<CalendarCard
															...
															} // Filtramos por el periodo del tab
															hasNavigation={
																false
															} // En vista anual no queremos botones prev/next
															className="h-full"
														/>
													);
												},
			...
			<aside className="mt-4">
				<Alert
					...
					description={
						<span>
							Fechas extraídas de la resolución:{" "}
							<b>RES - 2025 - 944 - CD-EXA # UNNE.</b>
						</span>
					}
				/>
			</aside>
