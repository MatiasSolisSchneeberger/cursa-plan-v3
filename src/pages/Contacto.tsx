...
const ETIQUETAS = [
	...
	{ value: "datos", label: "Error en Plan/Correlativas", icon: <IconBook size={16} /> },
	{ value: "sugerencia", label: "Sugerencia", icon: <IconBulb size={16} /> },
	{ value: "colaboracion", label: "Colaboración", icon: <IconUsers size={16} /> },
	{ value: "consulta", label: "Consulta General", icon: <IconQuestionMark size={16} /> },
];
		...
		>
			<div className="mb-6 flex flex-col items-center">
				...
				<TypographyP className="text-muted-foreground mt-2 max-w-2xl text-center">
					¿Tenés alguna duda sobre el plan de estudios? ¿Encontraste un 
					error en la plataforma? ¡Escribinos! Tu feedback nos ayuda a mejorar CursaPlan.
				</TypographyP>
			</div>
			...
			<section className="flex flex-col gap-6 md:flex-row">
				...
				<Card className="h-fit w-full md:w-1/3">
					<CardHeader>
						<CardTitle className="text-lg">Más Formas de Conectar</CardTitle>
					</CardHeader>
						...
						{SOCIAL_LINKS.map(({ href, icon, label }) => (
							...
							>
								<a href={href} target="_blank" rel="noopener noreferrer">
									<div className="flex h-5 w-5 items-center justify-center *:size-5">
										{icon}
									</div>
					...
					<CardContent>
						{status === "success" && (
							<Alert className="mb-6">
								...
								<AlertDescription>
									Recibimos tu mensaje correctamente y nos contactaremos pronto.
								</AlertDescription>
							</Alert>
						)}
						{status === "error" && (
						...
						<form onSubmit={handleSubmit} className="space-y-6">
							...
							<Field>
								...
								<Select
									...
									onValueChange={(val) => setFormData({ ...formData, etiqueta: val })}
									disabled={status === "loading"}
								>
