...
export default function Materia() {
	...
	const { isSoloCursar, isDesbloqueado } = getMateriaAvailability(
		correlativasFormat,
		getEstado,
	);

	let disponibilidadActual = disponibilidadMaterias.find(
		(d) => d.texto === "Bloqueado",
	);
	if (isSoloCursar)
		disponibilidadActual = disponibilidadMaterias.find(
			(d) => d.texto === "Solo Cursar",
		);
	if (isDesbloqueado)
		disponibilidadActual = disponibilidadMaterias.find(
			(d) => d.texto === "Desbloqueado",
		);

	const estadoConfig =
		estados.find((e) => e.texto === estadoActualTexto) || estados[0];
	...
	return (
		<section className="flex flex-col gap-3">
			...
			>
				{disponibilidadActual && (
					<Chip
						...
						title={
							!session
								? "Inicia sesión para ver tu disponibilidad"
								: "Disponibilidad para cursar/rendir"
						}
						disabled={!session}
					>
				...
				<Dropdown>
					<DropdownTrigger>
						<Button
							...
							title={
								!session
									? "Inicia sesión para cambiar tu estado"
									: "Cambiar estado"
							}
							disabled={!session}
						>
			...
			<Tabs
				defaultValue={
					tabsConfig.find((t: any) => t.default)?.id ||
					tabsConfig[0].id
				}
			>
				<ButtonGroup>
				...
				{tabsConfig.map((tab) => (
					<TabsContent key={tab.id} value={tab.id}>
						{tab.children ? (
							...
							<Tabs
								defaultValue={
									tab.children.find((c: any) => c.default)
										?.id || tab.children[0].id
								}
							>
								<ButtonGroup>
