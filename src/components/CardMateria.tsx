...
export default function CardMateria({
	...
	const { isBloqueado, isSoloCursar, isDesbloqueado } =
	...
	return (
		<Card className="">
			...
			<CardBody className="*:border-background-300 dark:*:border-background-700 flex flex-col gap-2 *:border-b-2 *:pb-2 *:last:border-b-0 *:last:pb-0">
				...
				<span className="flex flex-wrap gap-2">
					{disponibilidadMaterias.map((disponibilidad) => {
						...
						if (disponibilidad.texto === "Bloqueado")
							isSelected = isBloqueado;
						if (disponibilidad.texto === "Solo Cursar")
							isSelected = isSoloCursar;
						if (disponibilidad.texto === "Desbloqueado")
							isSelected = isDesbloqueado;
				...
				<span className="flex flex-wrap gap-2">
					{estados.map((estadoConfig) => {
						const isSelected =
							estadoActualTexto === estadoConfig.texto;
			...
			<CardFooter className="flex flex-col gap-2">
					...
					{materia.correlativas &&
						materia.correlativas.length > 0 && (
							<Button
								...
								className="w-full flex-1"
								onClick={() =>
									setShowCorrelativas(!showCorrelativas)
								}
							>
								{showCorrelativas
									? "Ocultar correlativas"
									: "Ver correlativas"}
							</Button>
						)}
				</section>
				...
				{showCorrelativas && (
					<CorrelativasList correlativas={materia.correlativas} />
				)}
			</CardFooter>
		</Card>
	);
