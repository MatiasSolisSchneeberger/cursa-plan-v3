...
export default function ButtonGroup({children, className}: {children: React.ReactNode; className?: string}) {
	...
	return (
		<motion.section
			ref={scrollRef}
			className={cn(
				"button-group w-full flex flex-row self-stretch shrink-0 relative h-min overflow-x-auto scroll-smooth no-scrollbar",
				"*:rounded-none",
			...
			style={{
				...
				transition: "mask-image 0.2s ease-out, -webkit-mask-image 0.2s ease-out", // Transición suave de la máscara
			}}>
			{children}
		</motion.section>
	)
