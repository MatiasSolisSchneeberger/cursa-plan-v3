...
export default function Select({
	...
	const selectedOption = groupOptions
	...
	return (
		<div
			...
			ref={containerRef}
		>
			<label className="texto-body text-text-800 dark:text-text-200 ml-1 font-medium">
				{label}
				{required && (
			...
			>
				<span
					className={cn(
						...
						!selectedOption
							? "text-text-600 dark:text-text-400"
				...
				<IconChevronDown
					size={18}
					className={cn(
						"text-text-500 absolute right-3 transition-transform duration-300",
						isOpen && "rotate-180",
					)}
				/>
			</button>
			...
			{isOpen && (
				<Menu className="custom-scrollbar border-background-200 dark:border-background-700 absolute top-full left-0 z-50 mt-2 w-full border">
					{groupOptions.map((group) => (
						<MenuGroup title={group.title}>
							...
							{group.options.length === 0 && (
								<MenuItem>No hay opciones disponibles</MenuItem>
							)}
						</MenuGroup>
					))}
				</Menu>
