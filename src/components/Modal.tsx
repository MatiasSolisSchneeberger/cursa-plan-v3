...
export function Modal({children, defaultOpen = false, onOpenChange}: ModalProps) {
	...
	return (
		<ModalContext.Provider value={{isOpen, setIsOpen: handleOpenChange, close: () => handleOpenChange(false)}}>
			{children}
...
export function ModalOpen({children, className}: {children: ReactNode; className?: string}) {
	const {setIsOpen} = useModal()

	return (
		<div onClick={() => setIsOpen(true)} className={cn("inline-block cursor-pointer", className)}>
			{children}
		</div>
	)
}
...
export function ModalClose({children, className}: {children: ReactNode; className?: string}) {
	...
	return (
		<div onClick={close} className={cn("inline-block cursor-pointer", className)}>
			{children}
		</div>
	)
}
			...
			{isOpen && (
				<div className="fixed inset-0 z-9999 flex items-center justify-center p-4">
					...
					<motion.div
						...
						className={cn("relative w-full max-h-[90vh]", sizes[size], className)}>
						{children}
					</motion.div>
				</div>
			)}
